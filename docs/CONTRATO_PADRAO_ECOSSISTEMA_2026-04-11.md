...
- este documento mistura dois niveis intencionais:
- `padrao vigente`, quando o ecossistema ja opera assim hoje;
- `padrao alvo`, quando a regra ainda precisa ser formalizada como contrato oficial.

Sempre que houver diferenca entre os dois, o documento deve explicitar isso.
...
Padrao vigente mais visivel hoje no ecossistema:

- `RECEIVED`
- `NOTIFIED`
- `WITHDRAWN`

Padrao alvo recomendado para fechamento futuro:
...
- enquanto `READY_FOR_WITHDRAWAL` nao existir oficialmente no backend, ele deve ser tratado como proposta de padrao alvo, nao como contrato vigente.
...
- enquanto `withdrawalQrCodeUrl` nao for o nome oficial publicado no backend, ele deve ser tratado como nome alvo de convergencia.
...
- a oficializacao dessa ordem depende de o backend publicar esses campos de forma consistente nos cenarios realmente suportados.
...
- essa estrutura deve ser tratada hoje como padrao alvo de governanca;
- so vira contrato vigente quando o backend publicar e aplicar isso oficialmente.
...
- essa tipagem ainda representa governanca desejada;
- so deve ser tratada como contrato implementado quando o backend publicar o stream oficial nesse formato.
...
### Regra 5. Criterio para desempate
